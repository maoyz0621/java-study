...
public class PdfReport {
    ...
    // main测试
    public static void gen(HttpServletResponse response, String fileName, List<String> tableHeaders, List<Object> tableVals) {
        // 1.新建document对象
        ...
        try {
            ...
            // 4.向文档中添加内容
            generatePdf(document, tableHeaders, tableVals);
            // 5.关闭文档
            document.close();
        } catch (Exception e) {
            log.error("", e);
    ...
    // 生成PDF文件
    public static void generatePdf(Document document, List<String> tableHeaders, List<Object> tableVals) throws Exception {
        ...
        for (String tableHeader : tableHeaders) {
            table.addCell(createCell(tableHeader, keyfont, Element.ALIGN_CENTER));
        }

        // 表格内容
        for (Object tableVal : tableVals) {
            table.addCell(createCell(tableVal.toString(), keyfont, Element.ALIGN_CENTER));
        }
